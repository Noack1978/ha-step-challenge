 ...
 * Native HA Custom Element – hass object passed directly, no token needed.
 * Archive data is read from sensor attributes (no REST API required).
 */
...
class StepChallengeCard extends HTMLElement {
  constructor() {
    ...
    this._tableRows   = 7;
    this._showOv      = false;   // settings overlay open
    this._ovTab       = 'main'; // 'main' | 'participants' | 'archive'
    this._selArchive  = new Set();
    this._addingPart  = false;   // show add-participant form in overlay
    this._archive     = [];      // loaded from status sensor attribute
  }
  ...
  _recTime() {
    // stored in options via config entry – read from status sensor extra attr
    const s = this._find('_status');
    return s?.attributes?.record_time || '23:00:00';
  }

  // Participants from leader sensor scores + stage sensor details
  _participants() {
    ...
    return Object.entries(leader.attributes.scores).map(([name, score]) => {
      ...
      return { name, score, key, steps: sensor?.attributes?.step_entity ? this._num(sensor.attributes.step_entity) : 0, wonDates: sensor?.attributes?.won_dates||[], stepEntity: sensor?.attributes?.step_entity||'' };
    });
  }

  // Archive is stored in status sensor attribute (added in sensor.py)
  _getArchive() {
    return (this._find('_status')?.attributes?.archive || []).slice().reverse();
  }
  ...
  _call(service, data={}) { this._hass?.callService(SC, service, data); }
  ...
  _render() {
    ...
    let h = `
      <div class="header">
        ${this._isPanel ? `<button class="icon-btn" id="menu-btn">☰</button>` : ''}
        <div class="hl">
          <h1>🏁 ${name}</h1>
          <div class="sub">Day ${elapsed} of ${total}</div>
        </div>
        <button class="icon-btn" id="settings-btn" title="Einstellungen">⚙️</button>
      ...
      <div class="btn-wrap">
        <button class="btn btn-track ${this._showTrack?'btn-on':''}" id="t">🗺 Route</button>
        <button class="btn btn-track ${this._showToday?'btn-on':''}" id="td">📍 Heute</button>
        <button class="btn btn-track" id="r">📋 Etappe werten</button>
      </div>`;

    if (status === 'inactive') {
      h += `<div class="empty"><div class="icon">🏃</div>
        <h2>Keine Challenge aktiv</h2>
        <p>Über ⚙️ eine neue Challenge starten.</p></div>`;
      this._paint(h);
      this._bind(parts, history, elapsed, total, start);
      return;
    }

    if (!parts.length) {
      h += `<div class="empty"><div class="icon">⚙️</div><h2>Lade…</h2></div>`;
      this._paint(h);
      this._bind(parts, history, elapsed, total, start);
      return;
    }
    ...
    if (status === 'finished') {
      h += `<div class="win-banner"><h2>🏆 ${parts[0].name} gewinnt!</h2>
        <p>${parts[0].score} von ${total} Etappen gewonnen</p></div>`;
    }
    ...
    // Table
    if (history.length>0) h += this._table(history.slice(-this._tableRows).reverse(), parts, history.length);
  ...
  _paint(html) {
    const ov = this._showOv ? this._renderOverlay() : '';
    this.shadowRoot.innerHTML = `<style>${CSS}</style><div class="root">${html}</div>${ov}`;
    this._bindOverlay();
    // Animate bars
    setTimeout(() => {
  ...
  _bind(parts, history, elapsed, total, start) {
    ...
    root.getElementById('settings-btn')?.addEventListener('click', () => {
      this._showOv=true; this._ovTab='main'; this._addingPart=false; this._render();
    });
    root.getElementById('t')?.addEventListener('click', () => { this._showTrack=!this._showTrack; this._render(); });
    root.getElementById('td')?.addEventListener('click', () => { this._showToday=!this._showToday; this._render(); });
    root.getElementById('r')?.addEventListener('click', () => this._call('record_day'));
    root.getElementById('rows-less')?.addEventListener('click', () => { this._tableRows=Math.max(1,this._tableRows-7); this._render(); });
    root.getElementById('rows-more')?.addEventListener('click', () => { this._tableRows=Math.min(history.length,this._tableRows+7); this._render(); });
    root.getElementById('rows-all')?.addEventListener('click',  () => { this._tableRows=history.length; this._render(); });
  }

  // ── Settings Overlay ──────────────────────────────────────────────────────

  _renderOverlay() {
    if (this._ovTab==='participants') return this._renderParticipants();
    if (this._ovTab==='archive')      return this._renderArchive();
    ...
    return `<div class="ov-bd" id="ov-bd">
      <div class="ov-box" onclick="event.stopPropagation()">
        ...
        <div class="ov-body">
          ...
          <input class="ov-in" id="ov-name" type="text" value="${name}">

          <label class="ov-lbl">Dauer (Tage)</label>
          <input class="ov-in" id="ov-days" type="number" min="7" max="365" value="${total}">

          <label class="ov-lbl">Auswertungszeit</label>
          <input class="ov-in" id="ov-time" type="time" value="${recTime.substring(0,5)}">
          ...
          <div class="ov-parts-preview">
            ${parts.map(p=>`<span class="ov-part-chip">${p.name}</span>`).join('')}
          </div>
          ...
          ${archive.length>0 ? `
            <div class="ov-arch-prev">
              ${archive.slice(0,3).map(c=>`
                <div class="ov-arch-row">
                  <b>${c.name}</b>
                  <span>${this._fmtD(c.start?.split('T')[0])} · 🏆 ${c.winner||'—'}</span>
                </div>`).join('')}
            </div>
  ...
  _renderParticipants() {
    const parts = this._participants();
    // Collect all daily_steps sensors for dropdown
    ...
    return `<div class="ov-bd" id="ov-bd">
      <div class="ov-box" onclick="event.stopPropagation()">
        ...
        <div class="ov-body">
          ...
          ${parts.map(p=>`
            <div class="ov-part-row">
              <div class="ov-part-info">
                <b>${p.name}</b>
                <span class="ov-hint">${p.stepEntity}</span>
              </div>
              <button class="ov-sm ov-danger" data-key="${p.key}" id="del-${p.key}">✕</button>
            </div>`).join('')}

          ${parts.length===0 ? `<div class="ov-hint">Keine Teilnehmer vorhanden.</div>` : ''}
  ...
  _renderArchive() {
    const archive = this._getArchive();
    const allSel  = this._selArchive.size===archive.length && archive.length>0;
    ...
    return `<div class="ov-bd" id="ov-bd">
      <div class="ov-box ov-wide" onclick="event.stopPropagation()">
        ...
        <div class="ov-body">
          ${archive.length===0 ? `<div class="ov-hint">Kein Archiv vorhanden.</div>` : `
  ...
  _bindOverlay() {
    const root = this.shadowRoot;
    if (!this._showOv) return;

    root.getElementById('ov-bd')?.addEventListener('click', e => {
      if (e.target.id==='ov-bd') { this._showOv=false; this._selArchive.clear(); this._render(); }
    });
    root.getElementById('ov-x')?.addEventListener('click', () => {
      this._showOv=false; this._ovTab='main'; this._selArchive.clear(); this._addingPart=false; this._render();
    });
    root.getElementById('ov-back')?.addEventListener('click', () => {
      this._ovTab='main'; this._addingPart=false; this._render();
    });

    // Main tab
    root.getElementById('ov-parts-btn')?.addEventListener('click', () => { this._ovTab='participants'; this._render(); });
    root.getElementById('ov-arch-btn')?.addEventListener('click',  () => { this._ovTab='archive'; this._selArchive.clear(); this._render(); });
    ...
    root.getElementById('ov-start')?.addEventListener('click', () => {
      ...
      setTimeout(() => { this._call('start'); this._showOv=false; this._render(); }, 300);
    });
    ...
    root.getElementById('ov-stop')?.addEventListener('click', () => {
      ...
      setTimeout(() => { this._call('stop'); this._showOv=false; this._render(); }, arch?400:0);
    });
    ...
    root.querySelectorAll('[id^="del-"]').forEach(btn => {
      ...
      btn.addEventListener('click', () => {
        ...
        if (confirm(`${key} entfernen?`)) this._call('remove_participant', { key });
      });
    });
    root.getElementById('add-btn')?.addEventListener('click', () => { this._addingPart=true; this._render(); });
    root.getElementById('add-cancel')?.addEventListener('click', () => { this._addingPart=false; this._render(); });
    ...
    root.getElementById('sel-all')?.addEventListener('change', e => {
      const arch=this._getArchive();
      if (e.target.checked) arch.forEach(c=>this._selArchive.add(c.id));
  ...
  _cal(elapsed, total, parts, history, startIso) {
    ...
    h+=`</div><div class="leg">`;
    parts.forEach((p,i)=>{h+=`<div class="leg-item"><div class="leg-dot" style="background:${COLORS[i%COLORS.length]}"></div>${p.name}</div>`;});
    return h+`</div></div>`;
  }

  // ── Table ─────────────────────────────────────────────────────────────────

  _table(recent, parts, totalCount) {
    let h=`<div class="sec">
      <div class="sec-label" style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <span style="font-size:.75rem;color:var(--secondary-text-color);min-width:80px;text-align:center">${Math.min(this._tableRows,totalCount)} / ${totalCount}</span>
          <button class="row-btn" id="rows-more" ${this._tableRows>=totalCount?'disabled':''}>+</button>
          <button class="row-btn" id="rows-all"  ${this._tableRows>=totalCount?'disabled':''}>All</button>
        </div>
      </div>
      <table><thead><tr><th>#</th><th>Datum</th><th>Sieger</th>`;
  ...
  _raceTrack(elapsed, total, parts, history, startIso) {
    ...
    for(let d=0;d<=total;d+=total<=31?1:5){if(d===0||d===total)markers+=`<text x="${toX(d).toFixed(1)}" y="${(PT+TH+14).toFixed(1)}" text-anchor="middle" font-size="8" fill="#6b6b8a">${d}</text>`;}
  ...
  _todayStage(elapsed, total, parts, history, startIso) {
    ...
    const segPx=[],segPy=[];
    for(let i=iFrom;i<=iTo;i++){segPx.push(PL+(i-elapsed+1)/1*TW/(2));segPy.push(PT+allElev[Math.min(i,total)]*TH);}
    const sFrom=Math.max(elapsed-1,0), sTo=elapsed;
    const stagePx=[],stagePy=[];
    for(let i=iFrom;i<=iTo;i++){stagePx.push(PL+((i-sFrom)/(sTo-sFrom))*TW);stagePy.push(PT+allElev[Math.min(i,total)]*TH);}
    ...
    const getY2=t=>{const seg=Math.min(Math.floor(t*(stagePx.length-1)),stagePx.length-2),lt=t*(stagePx.length-1)-seg,p0=Math.max(seg-1,0),p1=seg,p2=Math.min(seg+1,stagePx.length-1),p3=Math.min(seg+2,stagePx.length-1);const cp1y=stagePy[p1]+(stagePy[p2]-stagePy[p0])/6,cp2y=stagePy[p2]-(stagePy[p3]-stagePy[p1])/6,u=1-lt;return u*u*u*stagePy[p1]+3*u*u*lt*cp1y+3*u*lt*lt*cp2y+lt*lt*lt*stagePy[p2];};
    ...
    let dots2='',infoRows='';
    sorted3.forEach((p,i)=>{
      const sr=p.steps/maxSt, t=sr*tf, dx2=PL+t*TW, dy2=getY2(t), color=COLORS[parts.indexOf(p)%COLORS.length], isL=i===0, r=isL?10:7;
      const ly=i%2===0?dy2-r-7:dy2+r+13;
      dots2+=`<circle cx="${dx2.toFixed(1)}" cy="${dy2.toFixed(1)}" r="${r}" fill="${color}" stroke="var(--card-background-color)" stroke-width="2" ${isL?'filter="url(#glow2)"':''}/><text x="${dx2.toFixed(1)}" y="${ly.toFixed(1)}" text-anchor="${t>0.8?'end':t<0.2?'start':'middle'}" font-size="${isL?10:9}" font-weight="${isL?700:400}" fill="${color}">${p.name}</text>`;
...
const CSS = `
  ...
  /* ── Overlay ── */
  .ov-bd  { position:fixed; inset:0; background:rgba(0,0,0,.65); z-index:900; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(4px); }
